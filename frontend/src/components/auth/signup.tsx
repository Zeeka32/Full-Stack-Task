...
function Signup() {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    const isUsernameValid = usernameRegex.test(usernameForm);
    ...
    if(password !== passwordConfirm) {
      setPasswordConfirmErrorMessage("Passwords don't match");
      return false;
    }
    ...
    if (!response.ok) {
      setPasswordConfirmErrorMessage("Error making your account, try a different username");
      return;
    }
    ...
    <div className="w-10/12 sm:w-2/4">
      ...
      <form onSubmit={handleSubmit} className="bg-white shadow-xl border rounded px-8 pt-6 pb-8 mb-4 flex flex-col">
        ...
        <div className="mb-6">
          ...
          <label className="block text-grey-darker text-sm font-bold mb-2" htmlFor="passwordConfirm">
            Confirm Password
          </label>
          <input
            className="shadow appearance-none border rounded w-full py-2 px-3 text-grey-darker mb-2"
          ...
          />
          <p className="text-red-500 text-xs italic">{passwordConfirmErrorMessage}</p>
        </div>
